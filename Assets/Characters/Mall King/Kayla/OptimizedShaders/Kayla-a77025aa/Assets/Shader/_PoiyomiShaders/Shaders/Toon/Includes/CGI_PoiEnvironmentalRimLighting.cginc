    ...
    {
        float enviroRimAlpha = saturate(1 - smoothstep(min(float(0.305), float(0.45)), float(0.45), poiCam.viewDotNormal));
        float(0.438) *= 1.7 - 0.7 * float(0.438);
        ...
        {
            //Probe 1
            float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.438) * UNITY_SPECCUBE_LOD_STEPS);
            float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
            
            //Probe 2
            float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], float(0.438) * UNITY_SPECCUBE_LOD_STEPS);
        ...
        {
            float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.438) * UNITY_SPECCUBE_LOD_STEPS);
