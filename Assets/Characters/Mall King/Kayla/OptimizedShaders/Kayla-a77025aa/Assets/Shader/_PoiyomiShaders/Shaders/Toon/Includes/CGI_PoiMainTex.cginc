    ...
    void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
    {
        ...
        mainTexture.a *= alphaMask;
            ...
            float mixedHueShift = float(0.311);
            applyBackFaceTexture(backFaceDetailIntensity, mixedHueShift, albedo, backFaceEmission);
                ...
                if(float(1))
                {
                    albedo.rgb = lerp(albedo.rgb, hueShift(albedo.rgb, mixedHueShift + float(4.92) * _Time.x), hueShiftAlpha);
                }
                else
                {
                    albedo.rgb = hueShift(albedo.rgb, frac((mixedHueShift - (1 - hueShiftAlpha) + float(4.92) * _Time.x)));
                }
            #endif
            #ifdef FINALPASS
                half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).rgb * float4(1,1,1,1).rgb;
                albedo.rgb *= LerpWhiteTo(detailTexture * float(1) * unity_ColorSpaceDouble.rgb, detailMask.r * float(1.74) * backFaceDetailIntensity);
            #endif
