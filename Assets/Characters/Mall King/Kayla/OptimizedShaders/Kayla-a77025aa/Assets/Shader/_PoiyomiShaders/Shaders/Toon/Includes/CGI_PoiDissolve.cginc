    ...
    void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
    {
        ...
        if(float(1) == 1) // Basic
        {
            ...
            //noise = remap(noise, 0, 1, float(0.101), 1 - float(0.101));
            dissolveAlpha = dissolveAlpha >= noise;
            edgeAlpha = remapClamped(noise, da + float(0.101), da, 0, 1) * (1 - dissolveAlpha);
        }
        ...
        dissolveEmission = lerp(0, dissolveToTexture * float(10.3), dissolveAlpha) + lerp(0, edgeColor.rgb * float(10.01), smoothstep(0, 1 - float(0.5) * .99999999999, edgeAlpha));
    }
