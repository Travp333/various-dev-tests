    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = poiMesh.uv[0] * float(153.48);
        ...
        for (int j = -1; j <= 1; j ++)
        {
            for (int i = -1; i <= 1; i ++)
            {
                ...
                float2 rando = pos;
                pos = 0.5 + 0.5 * sin(float(0.706) * 6.2831 * pos);
        ...
        float size = float(0.641);
        
        if(float(1))
        {
            size = remapClamped(randomFromPoint, 0, 1, float4(0.2724295,0.6724296,0,1).x, float4(0.2724295,0.6724296,0,1).y);
        }
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(22)), 0);
