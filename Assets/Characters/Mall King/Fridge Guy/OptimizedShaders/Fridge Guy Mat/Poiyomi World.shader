Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/d36aa6b0323288445ac987011c8aaba4"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0,0,0,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(1));
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				float subsurface = 1;
				float skinScattering = saturate(subsurface * float(0.95) * 2);
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0,0,0,1), float4(0,0,0,1), float(1)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0,0,0,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				...
				if (float(0) == 1)
				{
					...
					float passthrough = 0;
					#else
					float passthrough = float(0.525);
					#endif
					if (float(0.415) == float(0.074)) float(0.415) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.415), float(0.074), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.074), float(0.415), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0.074), float(0.415), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(0) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.525) * poiLight.vAttenuation[index], smoothstep(float(0.074), float(0.415), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, float(0));
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, float(1.8));
				col = lerp(col, GetHDR(col), float(0));
				col = GetContrast(col, float(1));
				col *= float(1);
				col += float(0);
				float ppMask = 1;
				#if defined(PROP_PPMASK) || !defined(OPTIMIZER_ENABLED)
				ppMask = POI2D_SAMPLER_PAN(_PPMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				ppMask = lerp(ppMask, 1 - ppMask, float(0));
				col = lerp(poiFragData.finalColor, col, ppMask);
				#endif
				if (float(0))
				{
					col = lerp(col, poiPosterize(col, float(18.54)), ppMask);
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					poiLight.directColor = poiThemeColor(poiMods, float4(1,0,0,1), float(1));
				}
				#ifdef UNITY_PASS_FORWARDBASE
				...
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, float(0.525));
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(0));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(0));
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				meta.Emission = poiFragData.emission * float(0) * poiMods.globalEmission;
