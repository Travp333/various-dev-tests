Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/9a8f1cf7fd038b044baca2bd993240f9"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(1.5,5.5,0,0)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1.5,5.5,0,0)), float4(0,0,0,0)), float(4.42));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.07896994,0.07896994,0.07896994,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.07896994,0.07896994,0.07896994,1).a;
