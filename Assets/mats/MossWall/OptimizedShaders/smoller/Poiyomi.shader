Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/3fe14509c02772d4fbe111dcddce4faf"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			{
				float smoothness = float(0.321);
				float smoothness2 = float(1);
				float metallic = float(0.15);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.16));
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -float(0.57) * hueShiftAlpha.b);
