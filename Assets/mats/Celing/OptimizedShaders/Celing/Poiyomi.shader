Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/d0b81dd45280ce44f9242395ef14f4bc"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(1.8);
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				reflectionMask *= float(0.621);
				specularMask *= float(0.763);
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.13));
