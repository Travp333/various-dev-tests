Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/be1fa1e8fd6388a488854384d6ca2538"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void ApplyDetailColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * float(0) * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * float(0));
			}
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#endif
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0) * poiMods.detailMask.g);
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.1725488,1,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(4.7)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.1725488,1,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.1725488,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos, float4(16.9,-10,0,0).xyz) * float(22.67)) + _Time.x * float(10.54)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(16.9,-10,0,0).xyz, float(10.54), float(22.67), float(10), float(2.88), pos);
					}
				}
				if (float(0))
				{
					emissionStrength0 *= calculateBlinkingEmission(float(0), float(1), float(12.33), float(5.94));
				}
				applyLumaGradient(poiMods, emissionColor0, float(0), poiLight.nDotV);
				emissionColor0 = hueShift(emissionColor0, frac(float(0) + float(6.26) * _Time.x) * float(0));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.3));
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -float(-0.62) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(-0.296) * hueShiftAlpha.g);
				#endif
