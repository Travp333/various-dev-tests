Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/002944257dc2b1a4eb4f0beacc67343a"
{
	Properties
	{
 ...
 #define _SUNDISK_SIMPLE 
			...
			float3 RandomColorFromPoint(float2 rando)
			{
				...
				fixed saturation = lerp(float4(0.8000001,1,0,1).x, float4(0.8000001,1,0,1).y, rando.x);
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(213.9);
				...
				float size = float(0.209);
				
				if (float(0))
				{
					size = remapClamped(0, 1, randomFromPoint, float4(0,0.4000001,0,1).x, float4(0,0.4000001,0,1).y);
				}
				half glitterAlpha = 1;
				switch(float(1))
				{
					...
					if (float(0) == 1 || float(32.67) != 0)
					{
						...
						float theta = radians((randomBoy + _Time.x * float(32.67)) * 360);
						float cs = cos(theta);
						float sn = sin(theta);
				...
				half3 glitterColor = poiThemeColor(poiMods, float4(0,1,0.07410192,1), float(0));
				...
				switch(float(0))
				{
					...
					if (float(14.05) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(90), float(14.05));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0.003) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.97)), poiCam.viewDir)), float(300)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(14.05)) * float(4.8) - (float(4.8) - 1);
					finalGlitter = max(float(0.003) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
					break;
					case 2:
					if (float(14.05) > 0)
					{
					...
					finalGlitter = lerp(0, float(0.003) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(300)), 0);
				...
				if (float(0) == 1 || float(32.67) != 0 && !float(1))
				{
			...
			void applyMirror(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(0) != 0)
				{
					if (float(0) == 1 && !inMirror)	discard;
					if (float(0) == 2 && inMirror)	discard;
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1529262,1,0.1970753,0.1921569).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.1529262,1,0.1970753,0.1921569).a;
