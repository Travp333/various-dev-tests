Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/c6481a556e009ed4aa65306e74e6e3ac"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(1.66);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.764151,0.764151,0.764151,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.764151,0.764151,0.764151,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.764151,0.764151,0.764151,1).rgb, float(0));
				#endif
				if (float(0))
				{
