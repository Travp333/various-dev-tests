Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/937f6114537898144bc26e04926ab147"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			{
				float smoothness = float(0.789);
				float smoothness2 = float(1);
				float metallic = float(0.693);
				float specularMask = 1;
				float reflectionMask = 1;
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(8,8,0,0)), float4(0,0,0,0).xy);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(8,8,0,0)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(8,8,0,0)), float4(0,0,0,0)), float(0));
