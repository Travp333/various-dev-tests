Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/b446e7d23d382b54b99b5f7e3cf2b377"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			{
				float smoothness = float(0.356);
				float smoothness2 = float(1);
				float metallic = float(0.294);
				float specularMask = 1;
				float reflectionMask = 1;
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.7,0.7,0,0)), float4(0,0,0,0).xy);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(0.7,0.7,0,0)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.7,0.7,0,0)), float4(0,0,0,0)), float(0.52));
